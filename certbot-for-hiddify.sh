...
list_domains() {
    ...
    for i in "${!DOMAINS[@]}"; do
        ...
        if [[ -f "$OUTPUT_DIR/${DOMAINS[$i]}.crt" ]]; then
            cert_status=" ${GREEN}[✓ Certificate exists]${NC}"
        else
    ...
    case "$method" in
        ...
        3)
            ...
            if [[ ! -d "$webroot_path" ]]; then
                log "ERROR" "Webroot path does not exist: $webroot_path"
                return 1
            fi
            log "INFO" "Running webroot challenge..."
            if certbot certonly --webroot -w "$webroot_path" --non-interactive --agree-tos --register-unsafely-without-email --force-renewal -d "$domain" 2>&1 | tee -a "$LOG_FILE"; then
                certbot_success=true
            fi
            ;;
        *)
